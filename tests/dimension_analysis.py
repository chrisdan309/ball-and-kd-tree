...
def run_experiment(dimensions, n_points=1000, n_queries=200):
    ...
    for dim in dimensions:
        ...
        times["kdtree"].append(time.perf_counter() - start)

        custom_ball = CustomBallTree(data)
        start = time.perf_counter()
        for q in queries:
            custom_ball.nearest_neighbor(q)
        times["balltree"].append(time.perf_counter() - start)
...
def plot_results(times):
    ...
    plt.plot(times["dim"], times["kdtree"], label="KDTree", marker='o')
    plt.plot(times["dim"], times["balltree"], label="BallTree", marker='o')
